    ...
    const VisualizationParams &vis)
{
    ...
    if (current_lidar_seq != last_lidar_seq) {
        ...
        for (int angle = 0; angle < 360; angle++) {
            ...
            float x_mm = lidar_buffer[angle] * cos(Oradar_S2L_Grados_A_Radianes((float)angle));
            float y_mm = lidar_buffer[angle] * sin(Oradar_S2L_Grados_A_Radianes((float)angle));
        ...
        const float lambda_rad = Oradar_S2L_Grados_A_Radianes(bpd_lambda_deg);
        ...
        for (size_t i = 0; i < points_mm.size(); i++) {
            ...
            if (prev_inlier_idx >= 0) {
                float delta_phi_rad = Oradar_S2L_Grados_A_Radianes(point_angle_deg[i] - point_angle_deg[prev_inlier_idx]);
        ...
        if (next_segment_id > 0) {
            ...
            if (first_inlier >= 0 && last_inlier >= 0 && first_inlier != last_inlier) {
                float delta_phi_rad = Oradar_S2L_Grados_A_Radianes(360.0f - point_angle_deg[last_inlier] + point_angle_deg[first_inlier]);
...
void *Obstacle_Challenge_Thread(void *arg){
    ...
    //Oradar_S2L_Avanzar_Hasta_La_Distancia(50,0,150,Hold);
...
static float Fit_Line_Orientation(const std::vector<cv::Point2f> &pts, const std::vector<int> &indices) {
    ...
    return Oradar_S2L_Radianes_A_Grados(0.5 * atan2(2.0 * sxy, sxx - syy));
}
...
static float Line_Fit_Residual_Mm(const std::vector<cv::Point2f> &pts, const std::vector<int> &indices) {
    ...
    float theta_rad = Oradar_S2L_Grados_A_Radianes(Fit_Line_Orientation(pts, indices));
...
static WallSelection Select_Wall(const direction side) {
    ...
    for (int i = -edge_pad_deg; i < point + edge_pad_deg; i++) {
        ...
        if (d > min_range_mm) {
            pts.emplace_back(d * cos(Oradar_S2L_Grados_A_Radianes((float)wrapped_angle)),
                              d * sin(Oradar_S2L_Grados_A_Radianes((float)wrapped_angle)));
